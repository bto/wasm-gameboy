use super::*;

#[macro_use]
mod tests_macro;

#[test]
fn op_rl_a() {
    let mut cpu = CPU::new();
    let opcode = 0x17;

    // A(0b1000_0000), carry(false)
    let pc = cpu.registers.pc;
    set_inst!(cpu, pc, opcode);
    cpu.registers.a = 0b1000_0000;
    cpu.registers.carry = false;
    cpu.registers.half_carry = false;
    cpu.registers.subtraction = false;
    cpu.registers.zero = false;
    cpu.execute();
    assert_eq!(cpu.registers.pc, pc + 1);
    assert_eq!(cpu.registers.a, 0);
    assert_eq!(cpu.registers.carry, true);
    assert_eq!(cpu.registers.half_carry, false);
    assert_eq!(cpu.registers.subtraction, false);
    assert_eq!(cpu.registers.zero, true);

    // A(0b0110_1001), carry(true)
    let pc = cpu.registers.pc;
    set_inst!(cpu, pc, opcode);
    cpu.registers.a = 0b0110_1001;
    cpu.registers.carry = true;
    cpu.registers.half_carry = true;
    cpu.registers.subtraction = true;
    cpu.registers.zero = true;
    cpu.execute();
    assert_eq!(cpu.registers.pc, pc + 1);
    assert_eq!(cpu.registers.a, 0b1101_0011);
    assert_eq!(cpu.registers.carry, false);
    assert_eq!(cpu.registers.half_carry, false);
    assert_eq!(cpu.registers.subtraction, false);
    assert_eq!(cpu.registers.zero, false);
}

#[test]
fn op_rl_r() {
    let mut cpu = CPU::new();

    let opcode_base = 0b00010_000;
    for i in [0b000, 0b001, 0b010, 0b011, 0b100, 0b101, 0b111].iter() {
        let opcode = opcode_base | i;

        // 0b1000_0000, carry(false)
        let pc = cpu.registers.pc;
        set_inst!(cpu, pc, 0xCB, opcode);
        match i {
            0b000 => cpu.registers.b = 0b1000_0000,
            0b001 => cpu.registers.c = 0b1000_0000,
            0b010 => cpu.registers.d = 0b1000_0000,
            0b011 => cpu.registers.e = 0b1000_0000,
            0b100 => cpu.registers.h = 0b1000_0000,
            0b101 => cpu.registers.l = 0b1000_0000,
            0b111 => cpu.registers.a = 0b1000_0000,
            _ => panic!("never reach"),
        }
        cpu.registers.carry = false;
        cpu.registers.half_carry = false;
        cpu.registers.subtraction = false;
        cpu.registers.zero = false;
        cpu.execute();
        assert_eq!(cpu.registers.pc, pc + 2);
        assert_eq!(cpu.registers.carry, true);
        assert_eq!(cpu.registers.half_carry, false);
        assert_eq!(cpu.registers.subtraction, false);
        assert_eq!(cpu.registers.zero, true);
        match i {
            0b000 => assert_eq!(cpu.registers.b, 0),
            0b001 => assert_eq!(cpu.registers.c, 0),
            0b010 => assert_eq!(cpu.registers.d, 0),
            0b011 => assert_eq!(cpu.registers.e, 0),
            0b100 => assert_eq!(cpu.registers.h, 0),
            0b101 => assert_eq!(cpu.registers.l, 0),
            0b111 => assert_eq!(cpu.registers.a, 0),
            _ => panic!("never reach"),
        }

        // 0b0110_1001, carry(true)
        let pc = cpu.registers.pc;
        set_inst!(cpu, pc, 0xCB, opcode);
        match i {
            0b000 => cpu.registers.b = 0b0110_1001,
            0b001 => cpu.registers.c = 0b0110_1001,
            0b010 => cpu.registers.d = 0b0110_1001,
            0b011 => cpu.registers.e = 0b0110_1001,
            0b100 => cpu.registers.h = 0b0110_1001,
            0b101 => cpu.registers.l = 0b0110_1001,
            0b111 => cpu.registers.a = 0b0110_1001,
            _ => panic!("never reach"),
        }
        cpu.registers.carry = true;
        cpu.registers.half_carry = true;
        cpu.registers.subtraction = true;
        cpu.registers.zero = true;
        cpu.execute();
        assert_eq!(cpu.registers.pc, pc + 2);
        assert_eq!(cpu.registers.carry, false);
        assert_eq!(cpu.registers.half_carry, false);
        assert_eq!(cpu.registers.subtraction, false);
        assert_eq!(cpu.registers.zero, false);
        match i {
            0b000 => assert_eq!(cpu.registers.b, 0b1101_0011),
            0b001 => assert_eq!(cpu.registers.c, 0b1101_0011),
            0b010 => assert_eq!(cpu.registers.d, 0b1101_0011),
            0b011 => assert_eq!(cpu.registers.e, 0b1101_0011),
            0b100 => assert_eq!(cpu.registers.h, 0b1101_0011),
            0b101 => assert_eq!(cpu.registers.l, 0b1101_0011),
            0b111 => assert_eq!(cpu.registers.a, 0b1101_0011),
            _ => panic!("never reach"),
        }
    }
}

#[test]
fn op_rl_rrn() {
    let mut cpu = CPU::new();
    let opcode = 0x16;

    // 0b1000_0000, carry(false)
    let pc = cpu.registers.pc;
    set_inst!(cpu, pc, 0xCB, opcode);
    cpu.registers.h = 3;
    cpu.registers.l = 2;
    cpu.mmu.byte_set(0x302, 0b1000_0000);
    cpu.registers.carry = false;
    cpu.registers.half_carry = false;
    cpu.registers.subtraction = false;
    cpu.registers.zero = false;
    cpu.execute();
    assert_eq!(cpu.registers.pc, pc + 2);
    assert_eq!(cpu.mmu.byte_get(0x302), 0);
    assert_eq!(cpu.registers.carry, true);
    assert_eq!(cpu.registers.half_carry, false);
    assert_eq!(cpu.registers.subtraction, false);
    assert_eq!(cpu.registers.zero, true);

    // 0b0110_1001, carry(true)
    let pc = cpu.registers.pc;
    set_inst!(cpu, pc, 0xCB, opcode);
    cpu.registers.h = 4;
    cpu.registers.l = 3;
    cpu.mmu.byte_set(0x403, 0b0110_1001);
    cpu.registers.carry = true;
    cpu.registers.half_carry = true;
    cpu.registers.subtraction = true;
    cpu.registers.zero = true;
    cpu.execute();
    assert_eq!(cpu.registers.pc, pc + 2);
    assert_eq!(cpu.mmu.byte_get(0x403), 0b1101_0011);
    assert_eq!(cpu.registers.carry, false);
    assert_eq!(cpu.registers.half_carry, false);
    assert_eq!(cpu.registers.subtraction, false);
    assert_eq!(cpu.registers.zero, false);
}

#[test]
fn op_rlc_a() {
    let mut cpu = CPU::new();
    let opcode = 0x07;

    // 0
    let pc = cpu.registers.pc;
    set_inst!(cpu, pc, opcode);
    cpu.registers.a = 0;
    cpu.registers.carry = true;
    cpu.registers.half_carry = false;
    cpu.registers.subtraction = false;
    cpu.registers.zero = false;
    cpu.execute();
    assert_eq!(cpu.registers.pc, pc + 1);
    assert_eq!(cpu.registers.a, 0);
    assert_eq!(cpu.registers.carry, false);
    assert_eq!(cpu.registers.half_carry, false);
    assert_eq!(cpu.registers.subtraction, false);
    assert_eq!(cpu.registers.zero, true);

    // 0b1001_0110
    let pc = cpu.registers.pc;
    set_inst!(cpu, pc, opcode);
    cpu.registers.a = 0b1001_0110;
    cpu.registers.carry = false;
    cpu.registers.half_carry = true;
    cpu.registers.subtraction = true;
    cpu.registers.zero = true;
    cpu.execute();
    assert_eq!(cpu.registers.pc, pc + 1);
    assert_eq!(cpu.registers.a, 0b0010_1101);
    assert_eq!(cpu.registers.carry, true);
    assert_eq!(cpu.registers.half_carry, false);
    assert_eq!(cpu.registers.subtraction, false);
    assert_eq!(cpu.registers.zero, false);
}

#[test]
fn op_rlc_r() {
    let mut cpu = CPU::new();

    let opcode_base = 0b00000_000;
    for i in [0b000, 0b001, 0b010, 0b011, 0b100, 0b101, 0b111].iter() {
        let opcode = opcode_base | i;

        // 0
        let pc = cpu.registers.pc;
        set_inst!(cpu, pc, 0xCB, opcode);
        match i {
            0b000 => cpu.registers.b = 0,
            0b001 => cpu.registers.c = 0,
            0b010 => cpu.registers.d = 0,
            0b011 => cpu.registers.e = 0,
            0b100 => cpu.registers.h = 0,
            0b101 => cpu.registers.l = 0,
            0b111 => cpu.registers.a = 0,
            _ => panic!("never reach"),
        }
        cpu.registers.carry = true;
        cpu.registers.half_carry = false;
        cpu.registers.subtraction = false;
        cpu.registers.zero = false;
        cpu.execute();
        assert_eq!(cpu.registers.pc, pc + 2);
        assert_eq!(cpu.registers.carry, false);
        assert_eq!(cpu.registers.half_carry, false);
        assert_eq!(cpu.registers.subtraction, false);
        assert_eq!(cpu.registers.zero, true);
        match i {
            0b000 => assert_eq!(cpu.registers.b, 0),
            0b001 => assert_eq!(cpu.registers.c, 0),
            0b010 => assert_eq!(cpu.registers.d, 0),
            0b011 => assert_eq!(cpu.registers.e, 0),
            0b100 => assert_eq!(cpu.registers.h, 0),
            0b101 => assert_eq!(cpu.registers.l, 0),
            0b111 => assert_eq!(cpu.registers.a, 0),
            _ => panic!("never reach"),
        }

        // 0b1001_0110
        let pc = cpu.registers.pc;
        set_inst!(cpu, pc, 0xCB, opcode);
        match i {
            0b000 => cpu.registers.b = 0b1001_0110,
            0b001 => cpu.registers.c = 0b1001_0110,
            0b010 => cpu.registers.d = 0b1001_0110,
            0b011 => cpu.registers.e = 0b1001_0110,
            0b100 => cpu.registers.h = 0b1001_0110,
            0b101 => cpu.registers.l = 0b1001_0110,
            0b111 => cpu.registers.a = 0b1001_0110,
            _ => panic!("never reach"),
        }
        cpu.registers.carry = false;
        cpu.registers.half_carry = true;
        cpu.registers.subtraction = true;
        cpu.registers.zero = true;
        cpu.execute();
        assert_eq!(cpu.registers.pc, pc + 2);
        assert_eq!(cpu.registers.carry, true);
        assert_eq!(cpu.registers.half_carry, false);
        assert_eq!(cpu.registers.subtraction, false);
        assert_eq!(cpu.registers.zero, false);
        match i {
            0b000 => assert_eq!(cpu.registers.b, 0b0010_1101),
            0b001 => assert_eq!(cpu.registers.c, 0b0010_1101),
            0b010 => assert_eq!(cpu.registers.d, 0b0010_1101),
            0b011 => assert_eq!(cpu.registers.e, 0b0010_1101),
            0b100 => assert_eq!(cpu.registers.h, 0b0010_1101),
            0b101 => assert_eq!(cpu.registers.l, 0b0010_1101),
            0b111 => assert_eq!(cpu.registers.a, 0b0010_1101),
            _ => panic!("never reach"),
        }
    }
}

#[test]
fn op_rlc_rrn() {
    let mut cpu = CPU::new();
    let opcode = 0x06;

    // 0
    let pc = cpu.registers.pc;
    set_inst!(cpu, pc, 0xCB, opcode);
    cpu.registers.h = 3;
    cpu.registers.l = 2;
    cpu.mmu.byte_set(0x302, 0);
    cpu.registers.carry = true;
    cpu.registers.half_carry = false;
    cpu.registers.subtraction = false;
    cpu.registers.zero = false;
    cpu.execute();
    assert_eq!(cpu.registers.pc, pc + 2);
    assert_eq!(cpu.mmu.byte_get(0x302), 0);
    assert_eq!(cpu.registers.carry, false);
    assert_eq!(cpu.registers.half_carry, false);
    assert_eq!(cpu.registers.subtraction, false);
    assert_eq!(cpu.registers.zero, true);

    // 0b1001_0110
    let pc = cpu.registers.pc;
    set_inst!(cpu, pc, 0xCB, opcode);
    cpu.registers.h = 4;
    cpu.registers.l = 3;
    cpu.mmu.byte_set(0x403, 0b1001_0110);
    cpu.registers.carry = false;
    cpu.registers.half_carry = true;
    cpu.registers.subtraction = true;
    cpu.registers.zero = true;
    cpu.execute();
    assert_eq!(cpu.registers.pc, pc + 2);
    assert_eq!(cpu.mmu.byte_get(0x403), 0b0010_1101);
    assert_eq!(cpu.registers.carry, true);
    assert_eq!(cpu.registers.half_carry, false);
    assert_eq!(cpu.registers.subtraction, false);
    assert_eq!(cpu.registers.zero, false);
}

#[test]
fn op_rr_a() {
    let mut cpu = CPU::new();
    let opcode = 0x1F;

    // A(0b0000_0001), carry(false)
    let pc = cpu.registers.pc;
    set_inst!(cpu, pc, opcode);
    cpu.registers.a = 0b0000_0001;
    cpu.registers.carry = false;
    cpu.registers.half_carry = false;
    cpu.registers.subtraction = false;
    cpu.registers.zero = false;
    cpu.execute();
    assert_eq!(cpu.registers.pc, pc + 1);
    assert_eq!(cpu.registers.a, 0);
    assert_eq!(cpu.registers.carry, true);
    assert_eq!(cpu.registers.half_carry, false);
    assert_eq!(cpu.registers.subtraction, false);
    assert_eq!(cpu.registers.zero, true);

    // A(0b1001_0110), carry(true)
    let pc = cpu.registers.pc;
    set_inst!(cpu, pc, opcode);
    cpu.registers.a = 0b1001_0110;
    cpu.registers.carry = true;
    cpu.registers.half_carry = true;
    cpu.registers.subtraction = true;
    cpu.registers.zero = true;
    cpu.execute();
    assert_eq!(cpu.registers.pc, pc + 1);
    assert_eq!(cpu.registers.a, 0b1100_1011);
    assert_eq!(cpu.registers.carry, false);
    assert_eq!(cpu.registers.half_carry, false);
    assert_eq!(cpu.registers.subtraction, false);
    assert_eq!(cpu.registers.zero, false);
}

#[test]
fn op_rr_r() {
    let mut cpu = CPU::new();

    let opcode_base = 0b00011_000;
    for i in [0b000, 0b001, 0b010, 0b011, 0b100, 0b101, 0b111].iter() {
        let opcode = opcode_base | i;

        // 0b0000_0001, carry(false)
        let pc = cpu.registers.pc;
        set_inst!(cpu, pc, 0xCB, opcode);
        match i {
            0b000 => cpu.registers.b = 0b0000_0001,
            0b001 => cpu.registers.c = 0b0000_0001,
            0b010 => cpu.registers.d = 0b0000_0001,
            0b011 => cpu.registers.e = 0b0000_0001,
            0b100 => cpu.registers.h = 0b0000_0001,
            0b101 => cpu.registers.l = 0b0000_0001,
            0b111 => cpu.registers.a = 0b0000_0001,
            _ => panic!("never reach"),
        }
        cpu.registers.carry = false;
        cpu.registers.half_carry = false;
        cpu.registers.subtraction = false;
        cpu.registers.zero = false;
        cpu.execute();
        assert_eq!(cpu.registers.pc, pc + 2);
        assert_eq!(cpu.registers.carry, true);
        assert_eq!(cpu.registers.half_carry, false);
        assert_eq!(cpu.registers.subtraction, false);
        assert_eq!(cpu.registers.zero, true);
        match i {
            0b000 => assert_eq!(cpu.registers.b, 0),
            0b001 => assert_eq!(cpu.registers.c, 0),
            0b010 => assert_eq!(cpu.registers.d, 0),
            0b011 => assert_eq!(cpu.registers.e, 0),
            0b100 => assert_eq!(cpu.registers.h, 0),
            0b101 => assert_eq!(cpu.registers.l, 0),
            0b111 => assert_eq!(cpu.registers.a, 0),
            _ => panic!("never reach"),
        }

        // 0b1001_0110, carry(true)
        let pc = cpu.registers.pc;
        set_inst!(cpu, pc, 0xCB, opcode);
        match i {
            0b000 => cpu.registers.b = 0b1001_0110,
            0b001 => cpu.registers.c = 0b1001_0110,
            0b010 => cpu.registers.d = 0b1001_0110,
            0b011 => cpu.registers.e = 0b1001_0110,
            0b100 => cpu.registers.h = 0b1001_0110,
            0b101 => cpu.registers.l = 0b1001_0110,
            0b111 => cpu.registers.a = 0b1001_0110,
            _ => panic!("never reach"),
        }
        cpu.registers.carry = true;
        cpu.registers.half_carry = true;
        cpu.registers.subtraction = true;
        cpu.registers.zero = true;
        cpu.execute();
        assert_eq!(cpu.registers.pc, pc + 2);
        assert_eq!(cpu.registers.carry, false);
        assert_eq!(cpu.registers.half_carry, false);
        assert_eq!(cpu.registers.subtraction, false);
        assert_eq!(cpu.registers.zero, false);
        match i {
            0b000 => assert_eq!(cpu.registers.b, 0b1100_1011),
            0b001 => assert_eq!(cpu.registers.c, 0b1100_1011),
            0b010 => assert_eq!(cpu.registers.d, 0b1100_1011),
            0b011 => assert_eq!(cpu.registers.e, 0b1100_1011),
            0b100 => assert_eq!(cpu.registers.h, 0b1100_1011),
            0b101 => assert_eq!(cpu.registers.l, 0b1100_1011),
            0b111 => assert_eq!(cpu.registers.a, 0b1100_1011),
            _ => panic!("never reach"),
        }
    }
}

#[test]
fn op_rr_rrn() {
    let mut cpu = CPU::new();
    let opcode = 0x1E;

    // 0b0000_0001, carry(false)
    let pc = cpu.registers.pc;
    set_inst!(cpu, pc, 0xCB, opcode);
    cpu.registers.h = 3;
    cpu.registers.l = 2;
    cpu.mmu.byte_set(0x302, 0b0000_0001);
    cpu.registers.carry = false;
    cpu.registers.half_carry = false;
    cpu.registers.subtraction = false;
    cpu.registers.zero = false;
    cpu.execute();
    assert_eq!(cpu.registers.pc, pc + 2);
    assert_eq!(cpu.mmu.byte_get(0x302), 0);
    assert_eq!(cpu.registers.carry, true);
    assert_eq!(cpu.registers.half_carry, false);
    assert_eq!(cpu.registers.subtraction, false);
    assert_eq!(cpu.registers.zero, true);

    // 0b1001_0110, carry(true)
    let pc = cpu.registers.pc;
    set_inst!(cpu, pc, 0xCB, opcode);
    cpu.registers.h = 4;
    cpu.registers.l = 3;
    cpu.mmu.byte_set(0x403, 0b1001_0110);
    cpu.registers.carry = true;
    cpu.registers.half_carry = true;
    cpu.registers.subtraction = true;
    cpu.registers.zero = true;
    cpu.execute();
    assert_eq!(cpu.registers.pc, pc + 2);
    assert_eq!(cpu.mmu.byte_get(0x403), 0b1100_1011);
    assert_eq!(cpu.registers.carry, false);
    assert_eq!(cpu.registers.half_carry, false);
    assert_eq!(cpu.registers.subtraction, false);
    assert_eq!(cpu.registers.zero, false);
}

#[test]
fn op_rrc_a() {
    let mut cpu = CPU::new();
    let opcode = 0x0F;

    // 0
    let pc = cpu.registers.pc;
    set_inst!(cpu, pc, opcode);
    cpu.registers.a = 0;
    cpu.registers.carry = true;
    cpu.registers.half_carry = false;
    cpu.registers.subtraction = false;
    cpu.registers.zero = false;
    cpu.execute();
    assert_eq!(cpu.registers.pc, pc + 1);
    assert_eq!(cpu.registers.a, 0);
    assert_eq!(cpu.registers.carry, false);
    assert_eq!(cpu.registers.half_carry, false);
    assert_eq!(cpu.registers.subtraction, false);
    assert_eq!(cpu.registers.zero, true);

    // 0b0110_1001
    let pc = cpu.registers.pc;
    set_inst!(cpu, pc, opcode);
    cpu.registers.a = 0b0110_1001;
    cpu.registers.carry = false;
    cpu.registers.half_carry = true;
    cpu.registers.subtraction = true;
    cpu.registers.zero = true;
    cpu.execute();
    assert_eq!(cpu.registers.pc, pc + 1);
    assert_eq!(cpu.registers.a, 0b1011_0100);
    assert_eq!(cpu.registers.carry, true);
    assert_eq!(cpu.registers.half_carry, false);
    assert_eq!(cpu.registers.subtraction, false);
    assert_eq!(cpu.registers.zero, false);
}

#[test]
fn op_rrc_r() {
    let mut cpu = CPU::new();

    let opcode_base = 0b00001_000;
    for i in [0b000, 0b001, 0b010, 0b011, 0b100, 0b101, 0b111].iter() {
        let opcode = opcode_base | i;

        // 0
        let pc = cpu.registers.pc;
        set_inst!(cpu, pc, 0xCB, opcode);
        match i {
            0b000 => cpu.registers.b = 0,
            0b001 => cpu.registers.c = 0,
            0b010 => cpu.registers.d = 0,
            0b011 => cpu.registers.e = 0,
            0b100 => cpu.registers.h = 0,
            0b101 => cpu.registers.l = 0,
            0b111 => cpu.registers.a = 0,
            _ => panic!("never reach"),
        }
        cpu.registers.carry = true;
        cpu.registers.half_carry = false;
        cpu.registers.subtraction = false;
        cpu.registers.zero = false;
        cpu.execute();
        assert_eq!(cpu.registers.pc, pc + 2);
        assert_eq!(cpu.registers.carry, false);
        assert_eq!(cpu.registers.half_carry, false);
        assert_eq!(cpu.registers.subtraction, false);
        assert_eq!(cpu.registers.zero, true);
        match i {
            0b000 => assert_eq!(cpu.registers.b, 0),
            0b001 => assert_eq!(cpu.registers.c, 0),
            0b010 => assert_eq!(cpu.registers.d, 0),
            0b011 => assert_eq!(cpu.registers.e, 0),
            0b100 => assert_eq!(cpu.registers.h, 0),
            0b101 => assert_eq!(cpu.registers.l, 0),
            0b111 => assert_eq!(cpu.registers.a, 0),
            _ => panic!("never reach"),
        }

        // 0b0110_1001
        let pc = cpu.registers.pc;
        set_inst!(cpu, pc, 0xCB, opcode);
        match i {
            0b000 => cpu.registers.b = 0b0110_1001,
            0b001 => cpu.registers.c = 0b0110_1001,
            0b010 => cpu.registers.d = 0b0110_1001,
            0b011 => cpu.registers.e = 0b0110_1001,
            0b100 => cpu.registers.h = 0b0110_1001,
            0b101 => cpu.registers.l = 0b0110_1001,
            0b111 => cpu.registers.a = 0b0110_1001,
            _ => panic!("never reach"),
        }
        cpu.registers.carry = false;
        cpu.registers.half_carry = true;
        cpu.registers.subtraction = true;
        cpu.registers.zero = true;
        cpu.execute();
        assert_eq!(cpu.registers.pc, pc + 2);
        assert_eq!(cpu.registers.carry, true);
        assert_eq!(cpu.registers.half_carry, false);
        assert_eq!(cpu.registers.subtraction, false);
        assert_eq!(cpu.registers.zero, false);
        match i {
            0b000 => assert_eq!(cpu.registers.b, 0b1011_0100),
            0b001 => assert_eq!(cpu.registers.c, 0b1011_0100),
            0b010 => assert_eq!(cpu.registers.d, 0b1011_0100),
            0b011 => assert_eq!(cpu.registers.e, 0b1011_0100),
            0b100 => assert_eq!(cpu.registers.h, 0b1011_0100),
            0b101 => assert_eq!(cpu.registers.l, 0b1011_0100),
            0b111 => assert_eq!(cpu.registers.a, 0b1011_0100),
            _ => panic!("never reach"),
        }
    }
}

#[test]
fn op_rrc_rrn() {
    let mut cpu = CPU::new();
    let opcode = 0x0E;

    // 0
    let pc = cpu.registers.pc;
    set_inst!(cpu, pc, 0xCB, opcode);
    cpu.registers.h = 3;
    cpu.registers.l = 2;
    cpu.mmu.byte_set(0x302, 0);
    cpu.registers.carry = true;
    cpu.registers.half_carry = false;
    cpu.registers.subtraction = false;
    cpu.registers.zero = false;
    cpu.execute();
    assert_eq!(cpu.registers.pc, pc + 2);
    assert_eq!(cpu.mmu.byte_get(0x302), 0);
    assert_eq!(cpu.registers.carry, false);
    assert_eq!(cpu.registers.half_carry, false);
    assert_eq!(cpu.registers.subtraction, false);
    assert_eq!(cpu.registers.zero, true);

    // 0b0110_1001
    let pc = cpu.registers.pc;
    set_inst!(cpu, pc, 0xCB, opcode);
    cpu.registers.h = 4;
    cpu.registers.l = 3;
    cpu.mmu.byte_set(0x403, 0b0110_1001);
    cpu.registers.carry = false;
    cpu.registers.half_carry = true;
    cpu.registers.subtraction = true;
    cpu.registers.zero = true;
    cpu.execute();
    assert_eq!(cpu.registers.pc, pc + 2);
    assert_eq!(cpu.mmu.byte_get(0x403), 0b1011_0100);
    assert_eq!(cpu.registers.carry, true);
    assert_eq!(cpu.registers.half_carry, false);
    assert_eq!(cpu.registers.subtraction, false);
    assert_eq!(cpu.registers.zero, false);
}
